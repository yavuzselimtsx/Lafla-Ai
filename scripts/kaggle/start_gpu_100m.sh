...
LOG="$REPORTS/kaggle-gpu-launch.log"

mkdir -p "$WORK/data" "$REPORTS" "$WORK/tokenizer" "$CHECKPOINTS" "$HF_PACKAGE" "$ARCHIVES"
exec > >(tee -a "$LOG") 2>&1
...
test -s "$REPO/configs/data/source-plans/lafla-100m-source-plan.json" || {
...
tar -czf "$ARCHIVES/lafla-100m-thinking-kaggle-gpu-run.tar.gz" \
