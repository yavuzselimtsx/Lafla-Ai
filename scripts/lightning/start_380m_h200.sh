...
python -m lafla_ai_core.cli.preflight \
  ...
  configs/training/lightning/lightning-h200-380m-50000.yaml \
...
if [ ! -s "$RAW_DATA" ]; then
  python scripts/data/prepare_real_data.py \
    ...
    --identity-jsonl configs/data/identity/lafla-model-identity-380m.jsonl \
...
python -m lafla_ai_core.cli.tokenizer_train \
  ...
  configs/data/identity/lafla-model-identity-380m.jsonl \
  "$TOKENIZER_SAMPLE"
...
TRAIN_ARGS=(
  ...
  --model-config configs/model/lafla-380m-thinking.yaml
  --training-config configs/training/lightning/lightning-h200-380m-50000.yaml
  --tokenizer-path "$WORK/tokenizer/lafla-tokenizer.json"
  --checkpoint-dir "$WORK/checkpoints"
  --health-log "$REPORTS/train-health.jsonl"
  --data-jsonl configs/data/identity/lafla-model-identity-380m.jsonl
