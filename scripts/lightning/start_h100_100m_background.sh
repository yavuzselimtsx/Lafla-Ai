...
PID_FILE="$REPORTS/lightning-h100-100m.pid"
NOHUP_LOG="$REPORTS/lightning-h100-100m-nohup.log"
...
cd "$REPO"
nohup bash scripts/lightning/start_h100_100m.sh > "$NOHUP_LOG" 2>&1 &
PID="$!"
printf "%s\n" "$PID" > "$PID_FILE"
...
echo "[lafla] monitor: bash scripts/lightning/monitor_h100_100m.sh --watch 10"
